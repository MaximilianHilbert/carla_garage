...
def main():
            ...
            for repetition, checkpoint in repetition_dict.items():
                for weather in ["train", "test"]:
                    for town in ["Town01", "Town02"]:
                        ...
                        arguments = [
                            "--agent-yaml",
                            f'{os.path.join(os.environ.get("CONFIG_ROOT"), baseline, experiment, ".yaml")}',
                            "--baseline-name",
                            experiment,
                            ...
                            checkpoint,
                            "--eval_id",
                            f"{baseline}_{experiment}_{repetition}_{weather}_{town}",
                            "--log_path",
                            f"{os.path.join(os.path.dirname(os.path.dirname(checkpoint)), 'evaluation')}",
                            "--town",
                            town,
                            "--weather",
                            weather,
                            "--trafficManagerSeed",
                            str(next(traffic_seeds())),
                        ]
    ...
    for run_args in all_evaluation_arguments:
        ...
        process = subprocess.Popen(
            f'conda run -n garage python {os.path.join(os.environ.get("WORK_DIR"), "evaluate_nocrash_baselines.py")} {" ".join(run_args)}',
