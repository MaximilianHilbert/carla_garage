...
class BaseRenderer:
    ...
    def __init__(self, args, map_offset, map_dims, global_map=None):
        ...
        self.world_to_pix_crop_shift_tensor = torch.tensor(
            [0.0, -self.pixels_ahead_vehicle],
            device=self.args.device,
            dtype=torch.float32,
        )

        self.world_to_pix_crop_half_crop_dims = torch.tensor(
            [self.crop_dims[1] / 2, self.crop_dims[0] / 2], device=self.args.device
        )

        self.get_local_birdview_scale_transform = (
            torch.tensor(
    ...
    def get_local_birdview(self, position, orientation):
        ...
        translation_transform = torch.stack(
            [
                ones,
                zeros,
                position[..., 0:1] / self.crop_scale[0],
                zeros,
                ones,
                position[..., 1:2] / self.crop_scale[1],
                zeros,
                zeros,
                ones,
            ],
    ...
    def world_to_pix_crop(self, query_pos, crop_pos, crop_yaw):
        ...
        query_pos_px = torch.transpose(rotation, -2, -1) @ (query_pos_px_map - crop_pos_px).unsqueeze(-1)
