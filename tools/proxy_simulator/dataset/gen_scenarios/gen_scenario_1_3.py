"""
Geneates trigger locations for scenarios 1 and 3.
"""
...
def generate_scenario_3(carla_map):
    ...
    for _, waypoint in enumerate(topology):
        cur_wp = waypoint
        ...
        if len(wp_list_prev) + len(wp_list_nxt) > MIN_ROUTE_LENGTH:
            ...
            if -1 - DOT_PROD_SLACK < dot_ < -1 + DOT_PROD_SLACK:
                ...
                for i_ in range(len(final_wps_list)):
                    tmp_wps = final_wps_list[
                        i_ * NUM_WAYPOINTS_DISTANCE : i_ * NUM_WAYPOINTS_DISTANCE + NUM_WAYPOINTS_DISTANCE
                    ]
                    if len(tmp_wps) > 1:
...
FUNC_SCENARIO_TYPE = {"s1": generate_scenario_1, "s3": generate_scenario_3}


def town_scenario_tp_gen(
    town_, carla_map, scen_type, save_dir, world
