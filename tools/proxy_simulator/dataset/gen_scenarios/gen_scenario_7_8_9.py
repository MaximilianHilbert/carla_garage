"""
Geneates trigger locations for scenarios 7, 8 and 9.
"""
...
from utils_scenarios import interpolate_trajectory, gen_skeleton_dict, gen_scenarios
...
def generate_scenario_7_8_9(carla_map, world=None):
    ...
    trigger_points = {"s7": [], "s8": [], "s9": []}

    actors = world.get_actors()
    traffic_lights_list = actors.filter("*traffic_light")
    print(f"Got {len(traffic_lights_list)} traffic lights")
    ...
    for tl_ in traffic_lights_list:
        ...
        grp_tl_locs = [
            (
                round(gtl_.get_transform().location.x, 2),
                round(gtl_.get_transform().location.y, 2),
            )
            for gtl_ in grp_tl
        ]
    ...
    for grp_ in grps_:
        ...
        grp_bb_dist = [
            np.sqrt((bb_c[0] - midpt_grp[0]) ** 2 + (bb_c[1] - midpt_grp[1]) ** 2) for bb_c in junction_bbs_centers
        ]
        _, bb_idx = min((val, idx) for (idx, val) in enumerate(grp_bb_dist))
        bb_flags.append(bb_idx)

    signalized_junctions = [junc_ for i, junc_ in enumerate(junctions_) if i in bb_flags]
    signalized_junctions_bbs = [
        (junc_.bounding_box.location.x, junc_.bounding_box.location.y)
        for i, junc_ in enumerate(junctions_)
        if i in bb_flags
    ]
                ...
                if jbb_ in signalized_junctions_bbs:
                    ...
                    for j_wp in j_wps:
                        ...
                        while True:
                            ...
                            try:
                                # multiple routes are possible, we are only considering one possibility
                        ...
                        for wps_sub in truncated_wp_lst:
                            locations.append(
                                (
                                    wps_sub[0].transform.location.x,
                                    wps_sub[0].transform.location.y,
                                    wps_sub[-1].transform.location.x,
                                    wps_sub[-1].transform.location.y,
                                )
                            )
                        ...
                        for location_ in locations:
                            ...
                            for loc_dp in duplicate_list:
                                flag_ctrs = [
                                    (prev_loc - PRECISION <= curr_loc <= prev_loc + PRECISION)
                                    for curr_loc, prev_loc in zip(location_, loc_dp)
                                ]  # threshold hardset
                        ...
                        for j_, wps_ in enumerate(truncated_wp_lst):
                            if not are_loc_dups[j_]:
                                ...
                                wps_tmp = [
                                    wps_[0].transform.location,
                                    wps_[-1].transform.location,
                                ]
                                ...
                                if len(extended_route) < PRUNE_ROUTES_MIN_LEN:
                                    ...
                                    trigger_point = extended_route[2][
                                        0
                                    ]  # extended_route[int(len(extended_route)/2)][0]
                                    trigger_points[trig_key] += [trigger_point]

    other_actors = {
        "s7": [None] * len(trigger_points["s7"]),
        "s8": [None] * len(trigger_points["s8"]),
        "s9": [None] * len(trigger_points["s9"]),
    }
    return trigger_points, other_actors


FUNC_SCENARIO_TYPE = {"s7_8_9": generate_scenario_7_8_9}
...
def town_scenario_tp_gen(town_, carla_map, scen_type, save_dir, world):
    ...
    for key_ in ego_triggers_789.keys():
        ...
        for ego_trig, oa_trig in zip(ego_triggers, other_triggers):
            ...
            if ego_trig:
                ego_trig_dict = {
                    "pitch": ego_trig.rotation.pitch,
                    "yaw": ego_trig.rotation.yaw,
                    "x": ego_trig.location.x,
                    "y": ego_trig.location.y,
                    "z": ego_trig.location.z,
                }
        ...
        for town_dict_ in town_x_scen_y_dict["available_scenarios"]:
            if town_ in town_dict_.keys():
                for scens_dict in town_dict_[town_]:
        ...
        with open(
            os.path.join(save_dir[key_], f"{town_}_{scen_type}.json"),
            "w",
            encoding="utf-8",
        ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="mention single town, else generates for all towns",
    )
    ...
    args.save_dir = {
        "s7": os.path.join(args.save_dir, "s7"),
        "s8": os.path.join(args.save_dir, "s8"),
        "s9": os.path.join(args.save_dir, "s9"),
    }
