"""
Visualizes a route over a BEV image of a town.
"""
...
class MapImage(object):
    ...
    def __init__(self, carla_map_name, pixels_per_meter, map_dir, visualize=True):
        """
        Renders the map image generated based on the world, its map and additional flags that provide extra information
    ...
    def create_crop(self, surface, point_original, path_=None, np_array=False):
        ...
        forward_pt = [
            hero_location_screen[0] + hero_front.x * PIXELS_AHEAD_VEHICLE,
        ...
        final_surface.blit(
            window_map_surface,
            (0, 0),
            (top_left_in_pixels_final[0], top_left_in_pixels_final[1], 256, 256),
        )
        if path_:
        ...
        for j, pth_ in enumerate(points_):
            ...
            for i, (point_original) in enumerate(pth_[:-1]):
                ...
                pygame.draw.lines(
                    ...
                    color_white,
                    False,
                    [self.world_to_pixel(x.location) for x in [sp_, ep_]],
                    4,
                )
...
def main():
    ...
    argparser.add_argument(
        "--map_dir",
        default="../../leaderboard/data/maps",
        help="folder with created map images",
    )
    ...
    filename = Path(args.in_path).stem
    extension = Path(args.in_path).suffix
    ...
    if extension == ".xml":
        ...
        with open(args.in_path, encoding="utf-8") as json_file:
        ...
        for town_, scenarios_list in scenario_list.items():
            ...
            for scenario_ in scenarios_list:
                ...
                for scenario_trigger in scenario_all_triggers:
                    ...
                    sp_x, sp_y, sp_yaw = (
                        float(scenario_ego_spawn["x"]),
                        float(scenario_ego_spawn["y"]),
                        float(scenario_ego_spawn["yaw"]),
                    )
            ...
            map_image.plot_points_nocrop_json(
                map_image.viz_surface,
                args.points,
                filename + "_" + extension,
                args.save_dir,
