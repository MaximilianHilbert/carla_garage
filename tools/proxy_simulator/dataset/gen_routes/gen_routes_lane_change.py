"""
Generates training routes for lane changes.
"""
...
from utils_routes import ALL_TOWNS, interpolate_trajectory
...
def get_possible_lane_changes(current_waypoint):
    ...
    while True:
        ...
        if (
            ...
            and 0 <= abs(left_w.transform.rotation.yaw - tmp_wp.transform.rotation.yaw) <= 10
        ):
    ...
    while True:
        ...
        if (
            ...
            and 0 <= abs(right_w.transform.rotation.yaw - tmp_wp.transform.rotation.yaw) <= 10
        ):
            ...
            if len(wp_list_nxt) > wp_length:
                ...
                for wps_sub in truncated_wp_lst:
                    locations.append(
                        (
                            ...
                            wps_sub[0].transform.rotation.yaw,
                        )
                    )
                ...
                for location_ in locations:
                    ...
                    for loc_dp in duplicate_list:
                        flag_ctrs = [
                            (prev_loc - precision_small <= curr_loc <= prev_loc + precision_small)
        ...
        for key_ in ["rr", "lr", "ll", "rl"]:
            ...
            for ind_, child_ in enumerate(root[key_]):
                if ind_ in index_list_pruned:
                    route_new = ET.SubElement(
                        root_pruned[key_],
                        "route",
                        id=f"{route_id_pruned}",
                        town=args.town,
                    )
