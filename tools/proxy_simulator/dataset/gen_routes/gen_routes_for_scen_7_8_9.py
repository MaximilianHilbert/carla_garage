"""
Generates training routes given scenario definitions of type 7,8,9.
"""
import os
import argparse
...
def main():
    ...
    for town_ in towns:
        ...
        args.scenarios_file = {}
        args.save_file = {}
        for sceneraio_ in ["s7", "s8", "s9"]:
            args.scenarios_file[sceneraio_] = os.path.join(
                args.scenarios_dir, sceneraio_, town_ + "_" + sceneraio_ + ".json"
            )
        ...
        roots_ = {"s7": root_7, "s8": root_8, "s9": root_9}

        scenarios_list = {}
        scenarios_list["s7"] = parse_annotations_file(args.scenarios_file["s7"])
        scenarios_list["s8"] = parse_annotations_file(args.scenarios_file["s8"])
        scenarios_list["s9"] = parse_annotations_file(args.scenarios_file["s9"])
        ...
        if len(signalized_junctions):
            ...
            for waypoint in topology:
                if waypoint.is_junction:
                    junc_ = waypoint.get_junction()
                    jbb_ = (
                        junc_.bounding_box.location.x,
                        junc_.bounding_box.location.y,
                    )
