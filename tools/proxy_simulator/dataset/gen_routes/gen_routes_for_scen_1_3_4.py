                    ...
                    if -1 - dot_prod_slack < dot_ < -1 + dot_prod_slack:
                        ...
                        for i_ in range(len(final_wps_list)):
                            ...
                            if len(tmp_wps) > 1:
                                ...
                                norm_ = math.sqrt(
                                    vec_wp_nxt.x * vec_wp_nxt.x + vec_wp_nxt.y * vec_wp_nxt.y
                                ) * math.sqrt(vec_wp_prev.x * vec_wp_prev.x + vec_wp_prev.y * vec_wp_prev.y)
                            ...
                            for j_, wps_ in enumerate(truncated_wp_lst):
                                if not are_loc_dups[j_]:
                                    ...
                                    (
                                        ...
                                        _,
                                    ) = scan_route_for_scenarios(args.town, extended_route, scenarios_list)

                                    wps_ = [wps_[0], wps_[-1]]

                                    if (
                                        len(extended_route) <= MAX_LEN and len(potential_scenarios_definitions) > 0
                                    ) and len(extended_route) > prune_routes_min_len:
                                        route = ET.SubElement(
                                            root,
                                            "route",
                                            id=f"{route_id}",
