...
class OtherLeadingVehicle(BasicScenario):
    ...
    def __init__(
        ...
        super(OtherLeadingVehicle, self).__init__(
            "VehicleDeceleratingInMultiLaneSetUp",
            ego_vehicles,
            config,
            world,
            debug_mode,
    ...
    def _create_behavior(self):
        ...
        leading_actor_sequence_behavior.add_child(
            WaypointFollower(self.other_actors[0], decelerate, avoid_collision=True)
        )
        # end condition
        ...
        driving_in_same_direction.add_child(
            WaypointFollower(self.other_actors[1], self._second_vehicle_speed, avoid_collision=True)
        )
