...
torchrun --nnodes=1 --nproc_per_node=4 --rdzv_id=100 --rdzv_backend=c10d $TEAM_CODE/coil_train.py --use-disk-cache {args.use_disk_cache} --setting {args.setting} --dataset-repetition {args.dataset_repetition} --seed {seed} --batch-size {batch_size} --number-of-workers 4 --baseline-folder-name {baseline_folder_name} --training-repetition {training_repetition} \
...
export OPENBLAS_NUM_THREADS=1  # Shuts off numpy multithreading, to avoid threads spawning other threads.
torchrun --nnodes=1 --nproc_per_node=8 --rdzv_id=100 --rdzv_backend=c10d $TEAM_CODE/coil_train.py --use-disk-cache {args.use_disk_cache} --dataset-repetition {args.dataset_repetition} --setting {args.setting} --seed {seed} --batch-size {batch_size} --number-of-workers 8 --baseline-folder-name {baseline_folder_name} --training-repetition {training_repetition} \
"""+"--"+" --".join([f'{ablation} {" ".join(map(str,value)) if isinstance(value, list) else value}' for ablation, value in ablations_dict.items()])
            f.write(command)
