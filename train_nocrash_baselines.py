...
def generate_and_place_batch_script(args, seed, repetition):
    with open("/home/maximilian/Master/carla_garage/job.sh", 'w', encoding='utf-8') as f:
        command=f"""#!/bin/bash
...
conda run -n garage python3 $WORK_DIR/team_code/coil_train.py --gpu {args.gpu} --seed {seed} --training_repetition {repetition} --baseline_folder_name {args.baseline_folder_name} --baseline_name {args.baseline_name} --number_of_workers {args.number_of_workers}
        """
        f.write(command)
    subprocess.Popen(f"chmod +x /home/maximilian/Master/job_files/job_{args.baseline_folder_name, args.baseline_name, repetition}.sh", shell=True)
    subprocess.Popen(f'/home/maximilian/Master/job_files/job_{args.baseline_folder_name, args.baseline_name, repetition}.sh', shell=True)
