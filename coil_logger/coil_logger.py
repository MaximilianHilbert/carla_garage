...
def create_log(exp_batch_name: object, exp_name: object, training_rep:object,process_name: object, log_frequency: object = 1, image_log_frequency: object = 15) -> object:
    ...
    dir_name = os.path.join(root_path, exp_batch_name, exp_name, f"repetition_{str(training_rep)}")
    ...
    tl = Logger(os.path.join(root_path, exp_batch_name, exp_name,f"repetition_{str(training_rep)}",'tensorboard_logs_'+process_name))
...
def recover_loss_window(dataset_name, iteration):
    ...
    recovered_list = list(np.loadtxt(file_name).reshape(-1,))[0:iteration]
