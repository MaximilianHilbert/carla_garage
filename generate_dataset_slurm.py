...
def get_carla_command(gpu, num_try, start_port, carla_root):
    command = (
        f"SDL_VIDEODRIVER=offscreen SDL_HINT_CUDA_DEVICE={int(gpu)} {carla_root}/CarlaUE4.sh "
...
def create_run_eval_bash(
    ...
    with open(
        f"{bash_save_dir}/run_autopilot_{route}_Repetition{repetition}.sh",
        "w",
        encoding="utf-8",
    ) as rsh:
        rsh.write(
...
def main():
    ...
    date = "2024_08_30"
    dataset_name = "every_3rd_augmentation_rear_camera" + date
    ...
    for i in range(len(route_paths)):
                ...
                if fnmatch.fnmatch(name, route_pattern):
                    ...
                    if root.split("/")[-1].startswith("s"):
                        ...
                        if town == "10":
                            town = "10HD"
                        if os.path.exists(
                            f"{data_routes_dir}/scenarios/s{scenario}/Town{town}_Scenario{scenario}.json"
                        ):
                            scen_files.append(
                                f"{data_routes_dir}/scenarios/s{scenario}/Town{town}_Scenario{scenario}.json"
                            )
                        ...
                        assert os.path.exists(
                            f"{data_routes_dir}/scenarios/eval_scenarios.json"
        ...
        for ix, route in enumerate(route_files):
            ...
            num_running_jobs, max_num_parallel_jobs = get_num_jobs(
                job_name=exp_name, username=username, code_root=code_root
            )
    ...
    while not training_finished:
        ...
        for k in list(meta_jobs.keys()):
            ...
            if need_to_resubmit:
                ...
                os.system(
                    f"cp {log_root}/run_files/logs/qsub_err{job_nr_log}.log {log_root}/run_files/logs_{time_now_log}"
                )
                os.system(
                    f"cp {log_root}/run_files/logs/qsub_out{job_nr_log}.log {log_root}/run_files/logs_{time_now_log}"
                )
