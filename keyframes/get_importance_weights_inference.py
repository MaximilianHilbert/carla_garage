...
def main(args):
    ...
    action_prediction_model = ActionModel(input_dim=int(number_previous_actions)*merged_config_object.pred_len*2, output_dim=merged_config_object.number_future_waypoints*merged_config_object.pred_len*2, neurons=[int(neurons)])
    ...
    np.save(os.path.join(os.environ.get("WORK_DIR"), "_logs","keyframes", f"repetition_{str(args.repetition)}",f"bcoh_weights_prev{number_previous_actions}_rep{repetition}_neurons{neurons}.npy"),
            action_predict_losses)
