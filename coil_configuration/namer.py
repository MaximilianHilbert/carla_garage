...
def get_dropout_sum(model_configuration):
    return (
        sum(model_configuration["branches"]["fc"]["dropouts"])
...
def generate_name(g_conf):
    ...
    elif "res" in g_conf.MODEL_CONFIGURATION["perception"]:  # FOR NOW IT IS A RES MODEL
        final_name_string += "_" + str(g_conf.MODEL_CONFIGURATION["perception"]["res"]["name"])
    elif "beta-vae" in g_conf.MODEL_CONFIGURATION["perception"]:
        final_name_string += "_" + str(g_conf.MODEL_CONFIGURATION["perception"]["beta-vae"]["name"])
