    ...
    def plot_square(map_image, position, color, size):
        for i in range(0, size):
            for j in range(0, size):
                if (int(position[1]) + i) >= map_image.shape[0] or (int(position[0]) + j) >= map_image.shape[1]:
                    continue
...
def split_episodes(meas_file):
    ...
    previous_pos = [
        details_matrix[0, header_details.index("pos_x")],
    ...
    for i in range(1, len(details_matrix)):
        point = [
            details_matrix[i, header_details.index("pos_x")],
        ...
        if (
            previous_start_point != start_point and end_point != previous_end_point
        ) or repetition != previous_repetition:
...
def plot_episodes_tracks(exp_batch, experiment, checkpoint, town_name, exp_suite):
    ...
    results_dir = os.environ.get("COIL_RESULTS_PATH", "./")
    meas_file = os.path.join(
        results_dir,
        "_benchmarks_results",
        exp_batch + "_" + experiment + "_" + str(checkpoint) + "_drive_control_output_" + exp_suite + "_" + town_name,
        "measurements.csv",
    )
    ...
    for episode_vec in episodes_positions:
        ...
        plot_on_map(
            map_image,
            carla_map.convert_to_pixel(point),
            end_color_palete[int(end_cause[count])],
            16,
        )
