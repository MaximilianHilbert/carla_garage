...
def aggregate_metric(metric_raw, param):
    if np.isscalar(metric_raw):
        ...
        if param["type"] == "mean":
            ...
            metric_result = float(np.sum(metric_mask)) / float(
                len(metric_raw_np)
            )  # param['condition'] should be a lambda expression
        else:
...
def compute_km_per_infraction(data_item, param):
    label_to_index = {
        ...
        "intersection_otherlane": 2,
        "collision_pedestrians": 3,
        "collision_vehicles": 4,
    }
...
def compute_steering_classification_error(data_item, param):
    ...
    pred_np = np.digitize(pred_np, [-thresh, thresh])
    gt_np = np.digitize(gt_np, [-thresh, thresh])
    matches = 1.0 - (pred_np == gt_np).astype(np.float)
    return matches
...
def compute_displacement(data_item, param):
    return np.multiply(
        abs(data_item["steer_gt"] - data_item["steer_pred"]),
...
def compute_cumulative_displacement(data_item, param):
    ...
    for i in range(0, len(data_item["steer_pred"]) - param["window"]):
        displacement_vec_pred = [
            steer * math.fabs(speed) * param["timestep"]
            for steer, speed in zip(
                data_item["steer_pred"][(i) : (i + param["window"])],
        ...
        displacement_vec_gt = [
            steer * math.fabs(speed) * param["timestep"]
            for steer, speed in zip(
                data_item["steer_gt"][(i) : (i + param["window"])],
                data_item["speed_input"][(i) : (i + param["window"])],
            )
        ]
...
def compute_correlation(data_item, param):
    ...
    return calc_score(
        data_item["steer_pred"] * np.absolute(data_item["speed_input"]),
...
def compute_count_errors_weighted_speed(data_item, param):
    speed_weighted_coeff = param["coeff"] * np.sqrt(
        np.absolute(data_item["speed_input"]) * (1.0 / 40.0)
...
def compute_count_cumulative_displacement(data_item, param):
    ...
    for i in range(0, len(central_steer_error) - param["window"]):
        displacement_vec = [
            steer * speed * param["timestep"]
            for steer, speed in zip(
                central_steer_pred[(i) : (i + param["window"])],
        ...
        ground_truth_disp = [
            steer * speed * param["timestep"]
            for steer, speed in zip(
                central_steer_gt[(i) : (i + param["window"])],
                central_speed_input[(i) : (i + param["window"])],
            )
        ]
    ...
    return float(sum(cummulative_displacement_vec > (cummulative_ground_truth_disp * param["coeff"]))) / float(
