...
from coil_utils.general import static_vars
...
def augment_steering(camera_angle, steer, speed):
    ...
    val = 6 * (math.atan((rad_camera_angle * car_length) / (time_use * speed + 0.05))) / 3.1415
...
def get_speed_ground_truth(dataset_name):
    if dataset_name not in get_speed_ground_truth.previous_speed_ground_truth:
        full_path = os.path.join(os.environ["COIL_DATASET_PATH"], dataset_name, "speed_ground_truth.csv")
...
def get_camera_labels(dataset_name):
    if dataset_name not in get_camera_labels.previous_camera_labels:
        full_path = os.path.join(os.environ["COIL_DATASET_PATH"], dataset_name, "camera_labels.csv")
