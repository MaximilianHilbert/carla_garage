...
class PlanTAgent(DataAgent):
    ...
    def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        light_hazard = (
            torch.tensor(tick_data["light_hazard"], dtype=torch.int32).to(self.device).unsqueeze(0).unsqueeze(0)
        )
        stop_sign_hazard = (
            torch.tensor(tick_data["stop_sign_hazard"], dtype=torch.int32).to(self.device).unsqueeze(0).unsqueeze(0)
        )
        ...
        for i in range(self.model_count):
            pred_wp, pred_target_speed, pred_checkpoint, pred_bb = self.nets[i].forward(
