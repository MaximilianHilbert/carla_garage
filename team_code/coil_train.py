                ...
                if args.debug and epoch%1==0:
                    ...
                    if "arp" in merged_config_object.baseline_folder_name:
                        visualize_model(training=True,args=args,rgb=torch.cat([image for image in all_images[0]],axis=1).permute(1, 2, 0).detach().cpu().numpy(),
                                ...
                                velocity_vectors_pred=batch_of_bbs_pred[1] if batch_of_bbs_pred is not None else None,
                                acceleration_vectors_pred=batch_of_bbs_pred[2] if batch_of_bbs_pred is not None else None,
                                step=iteration,
                                gt_wp=targets[0],
                                pred_bb=batch_of_bbs_pred[0] if batch_of_bbs_pred is not None else None,
