        ...
        for epoch in tqdm(
            ...
            for iteration, data in enumerate(tqdm(data_loader, disable=rank != 0), start=1):
                ...
                if iteration>100 and iteration<150 and epoch==1:
                        timer.tic()
                if "bcso" in merged_config_object.baseline_folder_name:
                    pred_dict= model(x=all_images, vel_vecs=vel_vecs, accel_vecs=accel_vecs,speed=all_speeds if all_speeds is not None else None, target_point=target_point, prev_wp=additional_previous_waypoints)
                if "bcoh" in merged_config_object.baseline_folder_name or "keyframes" in merged_config_object.baseline_folder_name:
                    ...
                    if merged_config_object.batch_size==1:
                        if "arp" in merged_config_object.baseline_folder_name:
                            visualize_model(training=True,args=args,rgb=torch.cat([image for image in all_images.squeeze(0)],axis=1).permute(1, 2, 0).detach().cpu().numpy(),config=merged_config_object,
                                    ...
                                    velocity_vectors_gt=torch.squeeze(vel_vecs,0).detach().cpu().numpy(),
                                    acceleration_vectors_gt=torch.squeeze(accel_vecs, 0).detach().cpu().numpy(),
                                    velocity_vectors_pred=batch_of_bbs_pred[1].detach().cpu().numpy() ,
                                    acceleration_vectors_pred=batch_of_bbs_pred[2].detach().cpu().numpy(),
                                    step=iteration,
