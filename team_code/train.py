...
def main():
    ...
    parser.add_argument(
        "--use_controller_input_prediction",
        type=int,
    ...
    parser.add_argument(
        "--pred_len",
        type=int,
        default=config.pred_len,
        help="Number of waypoints the model predicts",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--estimate_semantic_distribution",
        type=int,
    ...
    parser.add_argument(
        "--bev_grid_height_downsample_factor",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="How many cpu cores are available on the machine." "The code will spawn a thread for each cpu.",
    )
    parser.add_argument(
        "--tp_attention",
        type=int,
    ...
    if config.use_cosine_schedule:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=config.cosine_t0, T_mult=config.cosine_t_mult, verbose=False
        )
    else:
        milestones = [args.schedule_reduce_epoch_01, args.schedule_reduce_epoch_02]
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones, gamma=config.multi_step_lr_decay, verbose=True
        )
    ...
    def load_data_compute_loss(self, data, validation=False):
        ...
        if self.config.use_plant:
            ...
            (
                ...
                bounding_boxes=bounding_box_label,
                route=route,
        ...
        if validation:
            ...
            if self.config.use_bev_semantic:
                ...
                bev_ss_miou = torchmetrics.functional.jaccard_index(
                    ...
                    task="multiclass",
                    ignore_index=-1,
                    num_classes=self.config.num_bev_semantic_classes,
                ).item()
        ...
        if (
            self.config.debug
            and (self.step % self.config.train_debug_save_freq == 0)
            and (self.vis_save_path is not None)
            and not self.config.use_plant
        ):
            with torch.no_grad():
