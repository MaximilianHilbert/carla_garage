...
class RelabelDataset(Dataset):  # pylint: disable=locally-disabled, invalid-name
    ...
    def __getitem__(self, index):
        ...
        for seq in range(0, num_seq):
            ...
            light_hazard = (
                torch.tensor(current_measurement["light_hazard"])
                .unsqueeze(0)
                .unsqueeze(0)
                .to(self.device, dtype=torch.int32)
            )
            stop_hazard = (
                torch.tensor(current_measurement["stop_sign_hazard"])
                .unsqueeze(0)
                .unsqueeze(0)
                .to(self.device, dtype=torch.int32)
            )
            junction = (
                torch.tensor(current_measurement["junction"])
                .unsqueeze(0)
                .unsqueeze(0)
                .to(self.device, dtype=torch.int32)
            )
            velocity = (
                torch.tensor(current_measurement["speed"])
                .unsqueeze(0)
                .unsqueeze(0)
            ...
            for i in range(2):
                ...
                bounding_boxes, _ = self.data_dummy.parse_bounding_boxes(
                    current_boxes,
                    None,
                ...
                bounding_boxes_padded = (
                    torch.tensor(bounding_boxes_padded).unsqueeze(0).to(self.device, dtype=torch.float32)
                )
                ...
                route = self.data_dummy.augment_route(
                ...
                target_point = self.data_dummy.augment_target_point(
                    target_point,
                    y_augmentation=aug_translation,
                    yaw_augmentation=aug_rotation,
                )
                ...
                if augment_sample:
                    ...
                    if self.args.debug:
                        ...
                        self.model.visualize_model(
                            save_path=self.args.debug_path,
                            step=index,
                    ...
                    pred_wp, pred_target_speed, pred_checkpoint, _ = self.model(
                        bounding_boxes=bounding_boxes_padded,
                        route=route,
                        target_point=target_point,
                        light_hazard=light_hazard,
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default=999,
        help="Number of repetitions to be " "relabelled",
    )
    parser.add_argument(
        "--debug",
        type=int,
        required=False,
        default=0,
        help="Whether to save debug data",
    )
    parser.add_argument(
        "--debug_path",
        type=str,
    ...
    relabel_dataset = RelabelDataset(
