...
class BevEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.perspective_upsample_factor = (
            self.image_encoder.feature_info.info[img_start_index + 2]["reduction"]
        ...
        if config.lidar_architecture == "video_resnet18":
            self.bev_encoder = VideoResNet(
                in_channels=1 + int(config.use_ground_plane) + self.config.bev_latent_dim,
                pretrained=False,
            )
            ...
            self.bev_encoder = SwinTransformer3D(
                pretrained=False,
                pretrained2d=False,
            ...
            self.bev_encoder = timm.create_model(
        ...
        self.upsampling_layer = UpsamplingConcat(
            num_img_features_1 + num_img_features_2,
    ...
    def forward(self, image, lidar):
        ...
        if len(self.bev_encoder.return_layers) > 4:
            fused_bev_features = self.forward_layer_block(
                bev_layers, self.bev_encoder.return_layers, fused_bev_features
            )
