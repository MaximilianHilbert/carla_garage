...
class PlanT(nn.Module):
    ...
    def __init__(self, config):
        ...
        precisions = [
            self.config.plant_precision_pos,
            self.config.plant_precision_pos,
            self.config.plant_precision_pos,
            self.config.plant_precision_pos,
            self.config.plant_precision_angle,
            self.config.plant_precision_speed,
        ...
        self.obj_token = nn.ParameterList(
            [
                nn.Parameter(torch.randn(1, self.config.plant_num_attributes))
                for _ in range(self.config.plant_object_types)
            ]
        )
        ...
        self.heads = nn.ModuleList(
            [nn.Linear(n_embd, self.vocab_size[i]) for i in range(self.config.plant_num_attributes)]
        )
        ...
        if self.config.use_controller_input_prediction:
            ...
            self.target_speed_network = nn.Sequential(
                nn.Linear(trans_out_features + 3, 128),
    ...
    def create_optimizer_groups(self, weight_decay):
        ...
        assert len(param_dict.keys() - union_params) == 0, (
            f"parameters {str(param_dict.keys() - union_params)} were not " f"separated into either decay/no_decay set!"
        )
    ...
    def forward(
        ...
        route_padding = torch.zeros(
            (bounding_boxes.shape[0], route.shape[1], 6),
            dtype=torch.float32,
    ...
    def control_pid(self, waypoints, speed, is_stuck):
        ...
        # future
        desired_speed = (
            np.linalg.norm(waypoints[int(self.config.carla_fps * 0.5)] - waypoints[self.config.carla_fps]) * 2.0
        )
    ...
    def visualize_model(  # pylint: disable=locally-disabled, unused-argument
        ...
        if gt_bbs is not None:
            ...
            for idx, box in enumerate(gt_bbs):
                ...
                box_img = t_u.bb_vehicle_to_image_system(
                    box, loc_pixels_per_meter, self.config.min_x, self.config.min_y
                )
                future_center_img = t_u.bb_vehicle_to_image_system(
                    future_center,
                    loc_pixels_per_meter,
                    self.config.min_x,
                    self.config.min_y,
                )
                color = color_classes[int(box[7])]
                bev_image = t_u.draw_box(
                    bev_image,
                    box_img,
                    color=color,
                    pixel_per_meter=loc_pixels_per_meter,
                )
                future_bev = t_u.draw_box(
                    future_bev,
                    future_center_img,
                    color=color,
                    pixel_per_meter=loc_pixels_per_meter,
        ...
        if gt_speed is not None:
            ...
            cv2.putText(
                bev_image,
                f"Car speed: {int(round(speed * 3.6))} km/h",
                (650, 1000),
        ...
        if stop_sign_hazard is not None:
            ...
            cv2.putText(
                bev_image,
                f"Stop sign?: {str(stop_sign_hazard)}",
                (50, 1000),
                cv2.FONT_HERSHEY_SIMPLEX,
                1.0,
                text_color,
