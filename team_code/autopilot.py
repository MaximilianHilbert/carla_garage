...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self._turn_controller_extrapolation = PIDController(
            k_p=self.config.turn_kp,
            k_i=self.config.turn_ki,
            k_d=self.config.turn_kd,
            n=self.config.turn_n,
    ...
    def _init(self, hd_map):
        ...
        self._waypoint_planner_extrapolation = RoutePlanner(
            self.config.dense_route_planner_min_distance,
            self.config.dense_route_planner_max_distance,
        )
    ...
    def run_step(
        self, input_data, timestamp, sensors=None, plant=False
    ...
    def _get_control(self, input_data, plant=False):
        ...
        if self.save_path is not None:
            ...
            route_logging = np.array([[node[0][0], node[0][1]] for node in waypoint_route_logging])[
    ...
    def _get_brake(self, near_command=None, plant=False):
        ...
        number_of_future_frames_no_junction = int(
            self.config.extrapolation_seconds_no_junction * self.config.bicycle_frame_rate
        )

        # Get future bbs of walkers
        if not plant:
            nearby_walkers = self.forcast_walkers(
                actors,
                ego_vehicle_location,
                ego_vehicle_transform,
                number_of_future_frames,
            ...
            for vehicle in vehicles:
                ...
                if vehicle.get_location().distance(ego_vehicle_location) < self.config.detection_radius:
                    ...
                    traffic_speed = self._get_forward_speed(
                        transform=traffic_transform, velocity=traffic_velocity
                    )  # In m/s
                    ...
                    if self.config.model_interactions:
                        ...
                        center_safety_box = traffic_transform.transform(
                            carla.Location(
                                x=traffic_safety_loc[0],
                                y=traffic_safety_loc[1],
                    ...
                    action = np.array(
                        [
                            traffic_control.steer,
                            traffic_control.throttle,
                            traffic_control.brake,
                        ]
                    )

                    traffic_safety_color = carla.Color(0, 255, 0, 255)
                    if self.config.model_interactions and self.check_obb_intersection(
                        traffic_safety_box, ego_bb_global
                    ):
                    ...
                    for i in range(number_of_future_frames):
                        ...
                        next_loc, next_yaw, next_speed = self.vehicle_model.forward(
                            next_loc, next_yaw, next_speed, action
                        )
                        ...
                        transform = carla.Transform(
                            ...
                            carla.Rotation(
                                pitch=traffic_transform.rotation.pitch,
                                yaw=delta_yaws,
                        ...
                        if self.config.model_interactions and (
                            self.check_obb_intersection(traffic_safety_box, bounding_boxes_back[i])
                            or self.check_obb_intersection(traffic_safety_box, bounding_boxes_front[i])
                        ):
                            traffic_safety_color = carla.Color(255, 0, 0, 255)
                            # Set action to break to model interactions
                            action[1] = 0.0
                    ...
                    if (
                        statistics.mean(self.vehicle_speed_buffer[vehicle.id]["velocity"])
                        < self.config.stuck_vel_threshold
                        and statistics.mean(self.vehicle_speed_buffer[vehicle.id]["throttle"])
                        > self.config.stuck_throttle_threshold
                        and statistics.mean(self.vehicle_speed_buffer[vehicle.id]["brake"])
                        < self.config.stuck_brake_threshold
                    ):
            ...
            for i, elem in enumerate(zip(bounding_boxes_front, bounding_boxes_back)):
                ...
                for vehicle_id, traffic_participant in nearby_vehicles.items():
                    ...
                    if vehicle_id in tmp_stucked_vehicle_id:
                        i_stuck = 0
                    back_intersect = (
                        self.check_obb_intersection(bounding_box_back, traffic_participant[i_stuck]) is True
                    )
        ...
        for o in range(min(index_drive_margin + index_future_orientation, number_of_future_frames)):
            ...
            next_loc_brake, next_yaw_brake, next_speed_brake = self.ego_model.forward(
                next_loc_brake, next_yaw_brake, next_speed_brake, action_brake
            )
            index = o
        ...
        center_safety_box = ego_vehicle_transform.transform(
            carla.Location(
                x=next_loc_brake[0],
                y=next_loc_brake[1],
                z=self._vehicle.bounding_box.location.z,
            )
        )
        ...
        # Stop sign detection
        # -----------------------------------------------------------
        self.stop_sign_hazard = self.ego_agent_affected_by_stop_sign(
            ego_vehicle_transform, ego_vehicle_location, actors, ego_speed, bounding_box
        )
    ...
    def forcast_ego_agent(
        ...
        for i in range(number_of_future_frames):
            ...
            steer_extrapolation_temp = self._get_steer_extrapolation(
                waypoint_route_extrapolation_temp,
                next_loc_no_brake,
                next_yaw_no_brake.item(),
                restore=False,
            )
            throttle_extrapolation_temp = self._get_throttle_extrapolation(
                self.target_speed, next_speed_no_brake, restore=False
            )
            brake_extrapolation_temp = 1.0 if self._waypoint_planner_extrapolation.is_last else 0.0
            action_no_brake = np.array(
                np.stack(
                    [
                        steer_extrapolation_temp,
                        float(throttle_extrapolation_temp),
                        brake_extrapolation_temp,
                    ],
                    axis=-1,
                )
            )
            ...
            transform = carla.Transform(
                carla.Location(
                    x=next_loc_no_brake[0].item() + extent.x * cosine,
            ...
            bounding_box_back.rotation = carla.Rotation(
                pitch=vehicle_transform.rotation.pitch,
                yaw=delta_yaws_no_brake,
                roll=vehicle_transform.rotation.roll,
            )
        ...
        for walker in walkers:
            if walker.get_location().distance(vehicle_location) < self.config.detection_radius:
                ...
                relative_pos = t_u.get_relative_transform(
                    np.array(vehicle_transform.get_matrix()),
                ...
                for i in range(number_of_future_frames):
                    ...
                    new_x = walker_location.x + (
                        walker_direction.x * walker_speed * (1.0 / self.config.bicycle_frame_rate)
                    )
                    new_y = walker_location.y + (
                        walker_direction.y * walker_speed * (1.0 / self.config.bicycle_frame_rate)
                    )
                    new_z = walker_location.z + (
                        walker_direction.z * walker_speed * (1.0 / self.config.bicycle_frame_rate)
                    )
    ...
    def ego_agent_affected_by_red_light(self, vehicle_transform, detection_box):
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            for wp in waypoints:
                ...
                affects_ego = False
                if self.check_obb_intersection(detection_box, bounding_box) or self.check_obb_intersection(
                    vehicle_bb, bounding_box
                ):
                    affects_ego = True
                    if light.state in (
                ...
                if self.visualize == 1:
                    ...
                    self._world.debug.draw_point(
                        wp.transform.location + carla.Location(z=light.trigger_volume.location.z),
                        size=0.1,
                        color=color,
                        life_time=0.01,
                    )
    ...
    def ego_agent_affected_by_stop_sign(self, vehicle_transform, vehicle_location, actors, speed, safety_box):
        ...
        vehicle_stop_sign_detector_bb = carla.BoundingBox(
            center_vehicle_stop_sign_detector_bb, extent_vehicle_stop_sign_detector_bb
        )
    ...
    def check_obb_intersection(self, obb1, obb2):
        ...
        return not (
            ...
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_right_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_up_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_forward_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_right_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_up_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_forward_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_right_vector()),
                obb1,
                obb2,
            )
            or self.get_separating_plane(
                r_pos,
                self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_up_vector()),
                obb1,
                obb2,
            )
        )
