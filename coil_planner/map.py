...
class CarlaMap(object):
    ...
    def convert_to_pixel(self, input_data):
        ...
        :return: A node object
        """
        pixel = self._converter.convert_to_pixel(input_data)
        if pixel[0] >= self.map_image_lanes.shape[1]:
            pixel[0] = self.map_image_lanes.shape[1] - 1
        if pixel[1] >= self.map_image_lanes.shape[0]:
            pixel[1] = self.map_image_lanes.shape[0] - 1
        return pixel
